...
| `/login` | `LoginComponent` | Authentification — écran de marque : fond plein écran `assets/images/login-background.svg` (SVG fait maison : dégradé bleu `#06468d`, trame comics, cases de planche, taches d'encre, silhouette du chien-plume reprise de `inkhound-logo-blue.svg`) et carte vitrée centrée (`backdrop-filter`, `data-coreui-theme="dark"` forcé pour rester identique en thème clair/sombre, bouton au bleu de marque via les variables `--cui-btn-*`). Styles dans `login.component.scss` |
